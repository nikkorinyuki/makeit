...
export async function generateImage(query: query) {
    ...
    const name = calc_best_size(query.name, maxTextWidth, canvasHeight, 20, { fonts: ["NotoSansJP-Medium", "NotoSansKR-Medium", "NotoSansSC-Medium"], italic: true, color: query.tcolor ?? "#000" }, query.markdown, 20);
    const id = calc_best_size(query.id, maxTextWidth, canvasHeight - name.totalHeight - margin_bottom, 20, { fonts: ["NotoSansJP-Medium", "NotoSansKR-Medium", "NotoSansSC-Medium"], color: "#8F8F8F" }, query.markdown, 20);
